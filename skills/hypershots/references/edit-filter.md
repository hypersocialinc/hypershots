...
**How the anchor works:** the lowest-numbered panel is graded first with no reference — that styled output becomes the set's anchor. Every remaining panel is then graded with the anchor passed as a style-reference image (panel first in `image_urls`, anchor second) and the prompt extended to match its grading exactly. Same prompt + shared anchor is what keeps N panels in ONE look instead of N drifting interpretations. **Verified live (full mode, Spotless 2-panel set):** anchored grading holds — both panels landed on the same kraft-paper tone (background channel means within ~4% of each other, matching speckle grain and identical flecked-ink letterforms), where the same style prompt without an anchor had drifted to a flat orange duotone in the earlier smoke test. On a mid-set failure the script stops, keeps the styled panels already written (non-destructive), and a re-run resumes safely — styled outputs are simply overwritten.

**Which mode a set makeover can actually use (empirical):** `protected` requires layouts with real unprotected background — frameless artifact panels, photo-bleed backgrounds, offset/partial devices. On the default centered layout (full-width copy block + large centered device) the padded protect union covers the whole canvas and `edit-pass.sh` refuses the first panel before spending anything — there, the theme restyle IS the makeover tool. `full` mode grades everything and holds the anchored look, but it redraws text and frame: headlines survive surprisingly well, small UI text goes soft/warped — expected and only acceptable if the user signs off on it panel by panel. Grades that keep each panel's overall palette still blend best in `protected` mode; hard palette shifts are `full` or a theme re-render.
